<x-layouts.app title="Confirm admin access">
    <div class="af-app-surface min-h-screen bg-zinc-50 dark:bg-zinc-950">
        <header class="af-page-header border-b border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950">
            <div class="mx-auto max-w-xl px-6 py-4">
                <p class="af-eyebrow">System Admin</p>
                <h1 class="text-xl font-semibold text-zinc-950 dark:text-zinc-50">Confirm admin access</h1>
                <p class="af-page-intro">Enter a fresh authenticator code or an unused recovery code before continuing to Administration.</p>
            </div>
        </header>

        <main class="mx-auto max-w-xl px-6 py-8">
            <form
                class="space-y-5 border-y border-zinc-200 py-6 dark:border-zinc-800"
                method="POST"
                action="{{ route('admin.two-factor.confirm.store') }}"
                data-two-factor-challenge
            >
                @csrf

                <div data-two-factor-authenticator-panel>
                    <label class="block" for="code">
                        <span class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Authentication code</span>
                        <input
                            id="code"
                            class="mt-1 w-full rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-950 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-50"
                            name="code"
                            type="text"
                            inputmode="numeric"
                            autocomplete="one-time-code"
                            autofocus
                            data-two-factor-authenticator-input
                        >
                    </label>
                    <p class="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
                        Codes are single-use. If this code just completed sign-in, wait for the next one.
                    </p>
                </div>

                <div data-two-factor-recovery-panel hidden id="admin-two-factor-recovery-panel">
                    <label class="block" for="recovery_code">
                        <span class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Recovery code</span>
                        <input
                            id="recovery_code"
                            class="mt-1 w-full rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-950 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-50"
                            name="recovery_code"
                            type="text"
                            autocomplete="off"
                            data-two-factor-recovery-input
                            disabled
                        >
                    </label>
                    <p class="mt-2 text-sm text-zinc-600 dark:text-zinc-400">Recovery codes are single-use.</p>
                </div>

                <button
                    class="af-auth-mode-toggle"
                    type="button"
                    data-two-factor-mode-toggle
                    aria-controls="admin-two-factor-recovery-panel"
                    aria-expanded="false"
                >
                    Use a recovery code
                </button>

                <noscript>
                    <div>
                        <label class="block" for="recovery_code_no_script">
                            <span class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Recovery code</span>
                            <input
                                id="recovery_code_no_script"
                                class="mt-1 w-full rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-950 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-50"
                                name="recovery_code"
                                type="text"
                                autocomplete="off"
                            >
                        </label>
                    </div>
                </noscript>

                @error('code')
                    <p class="text-sm text-red-700 dark:text-red-400">{{ $message }}</p>
                @enderror

                <div class="flex items-center gap-3">
                    <button class="af-primary-button" type="submit">
                        Confirm access
                    </button>
                    <a class="text-sm font-medium text-zinc-700 hover:text-zinc-950 dark:text-zinc-300 dark:hover:text-zinc-50" href="{{ route('dashboard') }}">
                        Back to overview
                    </a>
                </div>
            </form>
        </main>
    </div>
</x-layouts.app>
