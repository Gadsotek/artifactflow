...
The current MVP stays narrower and includes Markdown/wiki pages, single-file HTML artifact pages, normalized PNG/JPEG image or screenshot artifacts, and default-off searchable PDF, XLSX, and DOCX artifacts. Authenticated users can create Markdown pages with inline Mermaid diagrams, paste or upload single-file HTML artifacts, upload raster images whose metadata and non-pixel payloads are discarded by re-encoding, upload bounded PDFs whose embedded text is extracted by the dedicated processor, retain exact private XLSX originals whose visible content is projected into a bounded typed manifest by the dedicated SheetJS processor, and retain exact private DOCX originals whose dedicated LibreOffice processor produces a PDF that the independent PDFBox processor must validate as passive and natively searchable. HTML, scriptless image, typed XLSX, native PDF, and derived DOCX-PDF previews stay on the isolated artifact origin; native PDF and DOCX-PDF viewing are explicitly download-equivalent. Users can tag/search/version pages, control access through workspace roles and page overrides, and share internal authenticated links. The active external-sharing slice adds only the narrowly scoped page capabilities in the exclusive expiring or one-time modes defined by `docs/architecture/external-sharing.md`; do not present it as shipped until its required implementation and security proof are complete. Approved AI clients can retrieve authoritative source, normalized image content, bounded enveloped PDF/DOCX text, or bounded typed XLSX selections; preserve new text/HTML versions; create and replace normalized image or validated PDF/XLSX/DOCX artifacts through the dedicated `mcp:upload` boundary; update editable descriptions; organize title/hierarchy/category/tags through `mcp:organize`; and create narrowly scoped external shares for owned editable pages through `mcp:share`, under the same authorization, scanning, parser isolation, concurrency, and audit rules as the web interface. Initial parent assignment remains part of `mcp:create`; binary creation requires `mcp:create` plus `mcp:upload`, replacement requires `mcp:update` plus `mcp:upload`, and workspace creation/settings/membership/administration remain unavailable through MCP. PDF, XLSX, and DOCX remain default-off and may be enabled in production only with the containment and operator evidence required by their public architecture decisions; DOCX additionally requires the PDF processor boundary.
...
- Treat uploaded XLSX workbooks as untrusted ZIP/OPC/XML parser input. Process them only in the dedicated single-worker SheetJS container with bounded bytes/entries/expansion/XML/cells/formulas/links/time/memory/processes/tmpfs, no app source or storage credentials, HMAC-authenticated traffic, and no network. Retain the exact original privately, but preview, search, external sharing, and MCP reads may consume only the validated canonical typed manifest; never parse the workbook in Laravel or the browser.
- Treat uploaded DOCX documents as untrusted ZIP/OPC/XML and native-converter input. Admit only the reviewed passive WordprocessingML profile, convert it only in the dedicated networkless single-worker LibreOffice container, and submit the exact output bytes to the independent PDFBox DOCX-preview profile. Preview/search/sharing must fail closed unless the derivative is passive and contains bounded native text. Retain the exact original privately and make original download an explicit untrusted-file transfer.
...
- Artifact preview must use strict iframe sandboxing, strict CSP, signed short-lived URLs, and no app cookies on the artifact host. Browser-native viewing of an accepted PDF original or independently validated DOCX-derived PDF is the sole format-specific iframe-sandbox exception: those PDF responses may omit iframe and CSP `sandbox` controls only under `docs/architecture/pdf-artifacts.md` and `docs/architecture/docx-artifacts.md`, while retaining the separate cookieless artifact origin, PDF-specific restrictive CSP, signed revision-bound authorization, active-structure rejection, and browser security coverage. Never generalize this exception to HTML, images, or typed XLSX previews.
...
- External artifact sharing must follow `docs/architecture/external-sharing.md`: keep raw secrets out of request URLs and persistence, require explicit POST redemption for one-time links, use a distinct window-lived anonymous viewing session plus per-window proof, re-check live share/page state on every load, preserve every format's normal presentation boundary, expose only the typed XLSX manifest or validated DOCX-derived PDF for Office formats, and treat PDF/DOCX-PDF presentation as download-equivalent without adding an anonymous original-file download endpoint.
