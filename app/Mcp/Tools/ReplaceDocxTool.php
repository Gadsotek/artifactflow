...
use App\Application\Mcp\McpReplaceDocxTool as Handler;
...
#[Name('replace_docx')]
#[Description('Append a DOCX original to an editable Word document page with version concurrency and isolated processing.')]
final class ReplaceDocxTool extends ArtifactFlowTool
{
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            ...
            'base_version_uid' => $schema->string()->description('Must match the current version UID.')->required(),
    ...
    public function handle(Request $request): Response|\Laravel\Mcp\ResponseFactory
    {
        return $this->invoke(
            ...
            fn (User $actor, McpAccessToken $token, McpToolArguments $arguments): McpToolResult => $this->handler->handle(
                $actor,
                McpReplaceDocxInput::fromArguments($arguments, $this->provenanceArguments),
            ),
        );
    }
}
