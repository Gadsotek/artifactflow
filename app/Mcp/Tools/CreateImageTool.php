...
use App\Application\Mcp\McpCreateImageTool as Handler;
...
#[Name('create_image')]
#[Description('Create a PNG/JPEG image page after isolated normalization; submitted image bytes are discarded.')]
final class CreateImageTool extends ArtifactFlowTool
{
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            ...
            'image_base64' => $schema->string()
                ->description('Canonical standard Base64 PNG/JPEG bytes; data URLs and remote URLs are not accepted.')
                ->required(),
            'media_type' => $schema->string()->enum(['image/png', 'image/jpeg'])->required(),
            'change_summary' => $schema->string()
                ->description('Required concise summary of what this image version contains, up to 255 characters.')
                ->required(),
    ...
    public function handle(Request $request): Response|\Laravel\Mcp\ResponseFactory
    {
        return $this->invoke(
            ...
            fn (User $actor, McpAccessToken $token, McpToolArguments $arguments): McpToolResult => $this->handler->handle(
                $actor,
                $token,
                McpCreateImageInput::fromArguments($arguments, $this->provenanceArguments),
            ),
        );
    }
}
