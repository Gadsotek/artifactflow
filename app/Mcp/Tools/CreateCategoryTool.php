...
use App\Application\Mcp\McpCreateCategoryTool as Handler;
...
final class CreateCategoryTool extends ArtifactFlowTool
{
    ...
    public function handle(Request $request): Response|\Laravel\Mcp\ResponseFactory
    {
        return $this->invoke(
            ...
            fn (User $actor, McpAccessToken $token, McpToolArguments $arguments): McpToolResult => $this->handler->handle(
                $actor,
                McpCreateCategoryInput::fromArguments($arguments),
            ),
        );
    }
}
