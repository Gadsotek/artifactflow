...
- Version-level AI provenance keeps ArtifactFlow-observed ingest facts separate from optional producer claims. MCP clients can declare an exact provider/model ID and external origin reference without ArtifactFlow pretending to have verified it; restores retain byte lineage, and provenance survives ordinary version-content pruning.
...
- AI provenance: observed ingest facts, declared model claims, lineage, search, privacy, and retention are documented in the public architecture and artifact-lifecycle guides; detailed product and architecture decision records remain internal working documents.
