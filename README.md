...
The current public alpha supports Markdown pages, Mermaid diagrams, self-contained HTML artifacts, normalized PNG/JPEG screenshots or images, and default-off searchable PDF, Excel `.xlsx`, and Word `.docx` artifacts. Each binary document format requires its dedicated isolated processor boundary and explicit production opt-in; DOCX preview additionally depends on the PDF processor. See the [roadmap](ROADMAP.md).
...
- Opt-in XLSX workbooks retain the exact private original and use a separate networkless SheetJS CE processor to emit only a bounded canonical typed manifest. An application-owned read-only grid renders visible cells, cached formula results, merges, and safe typed links inside an opaque `allow-scripts` sandbox. External navigation is delegated to a destination-visible confirmation on the trusted app origin; neither Laravel nor the browser parses the workbook.
- Opt-in DOCX documents retain the exact private original. A networkless LibreOffice processor converts a sanitized copy whose external hyperlinks have become inert visible text, then the independent PDF processor permits only bounded internal links, rejects other active structures, and requires searchable native text. Only that validated derivative is previewed or externally shared; original download is an explicit transfer of untrusted Office bytes.
...
- Markdown/wiki pages with a rich editor over portable Markdown source, inline Mermaid diagrams (strict, no external calls), and authorization‑aware `[[Page Name]]` wiki links. Markdown, HTML, normalized raster images, and default-off PDF/XLSX/DOCX documents are the current artifact types within the vault; none defines the product category. The rich editor is a convenience over the **authoritative portable Markdown source**; switch to source view for byte‑exact control.
...
- Access managers can create either a reusable link with a required expiry or a one-time link that starts exactly one browser-window viewing session. Links remain revocable, follow the latest version, disclose no workspace/catalog/source/history surface, and preserve each format's normal presentation boundary. PDF uses its isolated native viewer, XLSX shares only the canonical typed manifest, and DOCX shares only the validated PDF derivative; there is no anonymous original-file download. PDF/DOCX viewing remains download-equivalent. The maximum expiring-link lifetime is configured by System Admins in whole days (1–30); one-time links remain usable until redeemed or revoked.
...
- An [MCP](https://modelcontextprotocol.io) server using the official Laravel MCP transport (app‑origin only) lets approved AI clients `list_workspaces` / `list_taxonomy` / `search` / `read` / `create` / `create_image` / `create_pdf` / `create_xlsx` / `create_docx` / `create_category` / `create_tag` / `organize` / `update` / `replace_image` / `replace_pdf` / `replace_xlsx` / `replace_docx` / `update_description` / `revert` / `create_external_share`, through the *same* handlers, policies, scanners, parser isolation, optimistic-concurrency checks, and audit trail as humans. Tool arguments and responses cross the application boundary through dedicated immutable DTOs and explicit whitelisted wire serializers; Laravel request/schema arrays and final JSON arrays remain at the adapter edge. `read` accepts optional `include` sections: omit it for content plus provenance, pass `[]` for metadata only, or select `content` and/or `provenance`. XLSX content reads additionally require an exact visible `xlsx_sheet` and canonical uppercase `xlsx_range` capped at 1,000 cells; the returned typed selection is independently capped at 2 MiB, while metadata-only workbook reads need no selector. Unrequested image bytes, storage reads, and provenance queries are skipped after normal authorization. `create` and the dedicated binary create tools accept a real `parent_page_uid`; `organize` can revision-safely change only title, parent, category, and tags. `mcp:organize` separately gates organization and taxonomy creation. Binary creation requires `mcp:create` plus `mcp:upload`, replacement requires `mcp:update` plus `mcp:upload`, and both accept only canonical bounded Base64 before the format-specific isolated processor. The server never fetches an upload URL. Image reads return normalized derivatives up to the configured `ARTIFACT_MAX_BYTES` (10 MiB by default, hard-capped at 64 MiB — the same read limit every page type shares, expanded by roughly a third once base64-framed) as a standard MCP image content block plus a machine-readable searchability hint; a retained derivative above that configured limit fails MCP read with `content_too_large` before its bytes are loaded or base64-expanded. Because pixels are not OCR-indexed, a missing description recommends `update_description`. Default-off PDF, XLSX, and DOCX reads return only enveloped selected/extracted content and safe facts—never original bytes, derived PDF bytes, storage paths, or signed URLs. MCP search includes XLSX/DOCX safe facts only when the token also has `mcp:read`; search-only tokens receive catalog metadata without those content-derived facts. Taxonomy discovery remains usage-visible: it returns only global tags attached to searchable pages and workspace-qualified categories the token can reach, with user-authored labels and slugs in explicit untrusted-data envelopes. Page search/read results include visibility-filtered parent, ancestor path, depth, and direct-child count metadata; inaccessible relatives remain undisclosed.
- MCP content writes accept optional, self-reported producer provenance. AI claims may be exact or partial: ArtifactFlow stores the reported provider alongside its normalized search key, accepts a best-known model family/label without requiring `model_id`, and computes completeness instead of rejecting useful fields. Short forward-compatible identity extensions are bounded and scanned; prompts, reasoning, credentials, authorization data, URLs, and content/blob payloads are rejected. MCP-reported client name/version stays separate, is not attested identity, and is never used to guess the model. Every successful create/update/image write returns `stored_provenance`, and server instructions tell the caller to disclose what was actually retained. Full `read` responses define each producer once in `producers`; `page_origin_producer_uids`, `direct_version_producer_uids`, and effective-origin UID references preserve lineage without repeating whole producer blocks. Absent optional descriptions, change summaries, producer identity fields, client fields, and external-reference values are omitted, while every present untrusted string keeps its complete field-level envelope. `search` can answer queries such as “everything Anthropic Opus produced” without exposing inaccessible pages.
- MCP initialization and write-tool schemas instruct AI clients to generate HTML artifacts as single self-contained documents with inline dependencies and no CDN or network-dependent features. Every MCP `create`, `update`, and `revert` content-version write requires a concise `change_summary`; `read` returns a present current summary in an explicit untrusted-data envelope. Description updates retain both concurrency values: the current-version UID binds an observation-derived description to the exact content or pixels inspected, while the metadata revision prevents overwriting a concurrent catalog edit.
...
The core local stack includes Reverb and isolated PDF, XLSX, and DOCX processors.
The guided installer keeps all three document capabilities disabled by default
and offers local/test `--pdf`, `--xlsx`, and `--docx` opt-ins. The interactive
wizard asks about Word first. Selecting DOCX enables its required PDF pipeline
without offering a contradictory standalone-PDF choice; declining DOCX leads to
the independent PDF question, followed by XLSX. Every processor uses socket-only
transport with no Docker network or public port.

XLSX previews intentionally use the same hashed, same-origin viewer assets as
production rather than loading JavaScript from the app's Vite origin. Run
`make build-assets` before enabling XLSX locally and again after changing its
viewer JavaScript or CSS; the artifact-host service ignores `public/hot` while
the app continues to use normal Vite development assets.
...
The wizard asks which environment you're setting up; choose **local** for this stack. It generates any missing application key, artifact signing key, and image-parser shared secret, then asks about the default-off document features in dependency order: DOCX first, standalone PDF only when DOCX was declined, and XLSX last. Selecting DOCX automatically enables its required PDF pipeline. The installer provisions a distinct processor secret for each selected boundary, runs migrations, prompts for your first System Admin, can add starter demo content (a Mermaid Markdown page plus an interactive HTML artifact), and runs the read-only doctor. Pass `--reverb` to provision and enable realtime locally. If the installer generates service configuration or changes any document setting, exit the container and rerun `make up` so Compose recreates the affected services with the persisted values, then run `make doctor`. XLSX is not ready until its signed live profile/containment check passes; DOCX is not ready until both its LibreOffice and downstream PDF checks pass. Then sign in at `http://localhost:18080/login`.
...
For an unattended local setup, pass `--env`, `--name`, `--email`, `--seed-demo`, and `--no-interaction` instead of answering prompts. Add `--pdf`, `--xlsx`, or `--docx` for the required document formats; `--docx` also enables PDF. Supply the first admin password through a mounted secret **file** — point `ARTIFACTFLOW_ADMIN_PASSWORD_FILE` at it (a single trailing newline is stripped). Unlike an inline `VAR=… command` assignment, a file leaks the secret to neither shell history nor the process argv:
...
php artisan artifactflow:install \
  ...
  --pdf --xlsx --docx --seed-demo --no-interaction
```
...
ArtifactFlow supports production self-hosting. `make build-prod` builds and runtime-tests the application plus its image, PDF, XLSX, and DOCX parser/processor images; `make scan-image` scans all of them. Tagged releases publish the application and the independently deployable PDF, XLSX, and DOCX processor images with separate digests, SBOMs, and provenance attestations. Run the application image with `APP_ENV=production`. The same application image runs the separate `app`, `artifact-host`, `worker`, and `scheduler` roles: `APP_RUNTIME_ROLE` selects the role, and the `worker`/`scheduler` roles additionally override the container command to their start script (see the [operations guide](docs/OPERATIONS.md)).
...
- Default-off Excel workbook support: the public [XLSX architecture decision](docs/architecture/xlsx-artifacts.md) defines strict ZIP/OPC admission, SheetJS typed projection, the read-only sandboxed viewer, lifecycle/search/sharing/MCP behavior, and its production gates.
- Default-off Word document support: the public [DOCX architecture decision](docs/architecture/docx-artifacts.md) defines strict package admission, isolated LibreOffice conversion, independent PDF validation, native-text acceptance, derived-only sharing, and the dual-processor production gates.
...
make type-coverage   # 100%         make audit        # advisories + license policy
make coverage        # PCOV line coverage (95% floor)
...
Note the distinction: **type** coverage is enforced at **100%**; **line** coverage is gated at a **95% floor** (`COVERAGE_MIN`), via CI's `make coverage`, not `phpunit.xml`.
...
Dependencies whose binaries or bundled browser code are redistributed with
ArtifactFlow are recorded in [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md).
`make audit` also checks every locked Composer and npm package against the
reviewed license policy in
[`security/dependency-license-policy.json`](security/dependency-license-policy.json).
The same fail-closed check runs in the nightly dependency audit; a missing,
unparsable, or newly incompatible license requires explicit review.
