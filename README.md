...
- Opt-in XLSX workbooks retain the exact private original and use a separate networkless SheetJS CE processor to emit only a bounded canonical typed manifest. An application-owned read-only grid renders visible cells, cached formula results, merges, and safe typed links inside an opaque sandbox; neither Laravel nor the browser parses the workbook.
- Opt-in DOCX documents retain the exact private original. A networkless LibreOffice processor creates a PDF, then the independent PDF processor rejects active structures and requires searchable native text. Only that validated derivative is previewed or externally shared; original download is an explicit transfer of untrusted Office bytes.
